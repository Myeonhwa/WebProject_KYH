...
//		DAO의 메서드를 호출하여 목록에 출력할 게시물을 얻어온다.
		List<COMMENTBoardDTO> boardLists = dao.selectListPage(map);
		System.out.println(boardLists);
		dao.close();
		
//		뷰에 전달할 매개변수 추가
//		목록 하단에 출력할 페이지 바로가기 링크를 얻어온 후 Map에 추가
		String pagingImg = BoardPage.pagingStr(totalCount, pageSize, blockPage, pageNum, "../commentboard/commentListPage.do");
...
//		전달할 데이터를 request 영역에 저장 후 View로 포워드
		req.setAttribute("commentLists", boardLists);
